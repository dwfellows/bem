...
m_choice = '15'; amp_choice = 'origamp';  % choices: 05, 085, 11, 15; amp: orig/twice/four
M = str2num(strcat([m_choice(1), '.', m_choice(2:end)]));
amp = 0.0000508; % amp = 0.000722157;
omega = 691.15038379; % omega = 376.991;
dmd_on = 1;

if (strcmp(amp_choice,'twiceamp'))
  amp = amp*2;
elseif (strcmp(amp_choice,'fouramp'))
  amp = amp*4;
elseif (strcmp(amp_choice,'tenamp'))
  amp = amp*10;
elseif (strcmp(amp_choice,'twentyamp'))
  amp = amp*20;
elseif (strcmp(amp_choice,'fiftyamp'))
  amp = amp*50;
end
...
La = 0.2286; Lb = 2*La;
flow.L = La; flow.La = La; flow.Lb = Lb;
Nx = 91; Ny = 181;
% Nx = 81; Ny = 161;  % subsonic 3D resolution for thesis
N = (Nx-1)*(Ny-1);
...
figure;
scatter3(col_pts(:,1) ./ La, col_pts(:,2) ./ La, psi_in, sz, psi_in); grid on;
...
figure;
scatter3(col_pts(:,1) ./ La, col_pts(:,2) ./ La, dpsi_dx_in, sz, dpsi_dx_in); grid on;
colorbar; colormap('jet'); pbaspect([1 2 1]); view(2);
ax = gca; ax.FontSize = afs;
xlabel('$x / a$', 'interpreter', 'latex', 'fontsize', fs);
ylabel('$y / a$', 'interpreter', 'latex', 'fontsize', fs);
...
%   subplot(221);
%   scatter(col_pts(:,1) ./ La, col_pts(:,2) ./ La, sz, real(bem_mode)); grid on;
...
% 
%   subplot(222);
%   scatter(col_pts(:,1) ./ La, col_pts(:,2) ./ La, sz, imag(bem_mode)); grid on;
%   ax = gca; ax.FontSize = afs; colorbar; colormap('jet'); pbaspect([1 2 1]);
%   ylabel('$y / a$', 'interpreter', 'latex', 'fontsize', fs);
...
pf_bem_history = (pbar_prime_3d_anal.') * exp(i*omega*t_phys); pf_bem_history = (pf_bem_history + conj(pf_bem_history)) ./ flow.pref;
p_bar_lpt = (-i*(amp/2)*flow.gamma*flow.M.*dpsi_dx_in + flow.gamma*k_wav*(amp/2).*psi_in);
pf_lpt_history = (p_bar_lpt) * exp(i*omega*t_phys); pf_lpt_history = pf_lpt_history + conj(pf_lpt_history);
maxpfbem = max(max(abs(pf_bem_history)));
maxpflpt = max(max(pf_lpt_history));
disp(strcat(['p-prime max BEM ', num2str(maxpfbem)]));
disp(strcat(['p-prime max PT: ', num2str(maxpflpt)]));

figure;
subplot(121); scatter(col_pts(:,1), col_pts(:,2), 24, real(pbar_prime_3d_anal)./flow.pref); grid on; title('Re');
colorbar; colormap('jet');
subplot(122); scatter(col_pts(:,1), col_pts(:,2), 24, imag(pbar_prime_3d_anal)./flow.pref); grid on; title('Im');
colorbar; colormap('jet');
sgtitle('BEM');
figure;
subplot(121); scatter(col_pts(:,1), col_pts(:,2), 24, real(p_bar_lpt)); grid on; title('Re');
colorbar; colormap('jet');
subplot(122); scatter(col_pts(:,1), col_pts(:,2), 24, imag(p_bar_lpt)); grid on; title('Im');
colorbar; colormap('jet');
sgtitle('PT');

if (M<1)
  pressure_data = dlmread(strcat(['./comparison_data/c6_3d/c6_3d_subsonic_data_streamref/m', m_choice, '/', amp_choice, '/surface_pressure_history_c6_m', m_choice, '_streamref_', amp_choice, '.csv']));
  x_data = dlmread(strcat(['./comparison_data/c6_3d/c6_3d_subsonic_data_streamref/m', m_choice, '/', amp_choice, '/surface_xloc_history_c6_m', m_choice, '_streamref_', amp_choice, '.csv']));
  y_data = dlmread(strcat(['./comparison_data/c6_3d/c6_3d_subsonic_data_streamref/m', m_choice, '/', amp_choice, '/surface_yloc_history_c6_m', m_choice, '_streamref_', amp_choice, '.csv']));
  z_data = dlmread(strcat(['./comparison_data/c6_3d/c6_3d_subsonic_data_streamref/m', m_choice, '/', amp_choice, '/surface_zloc_history_c6_m', m_choice, '_streamref_', amp_choice, '.csv']));
else
...
pf_cfd = pressure_data - 1;
[maxpfcfd, maxpfcfdind] = max(max(abs(pf_cfd)));
pf_metric = ((maxpfbem - maxpfcfd)/maxpfcfd)*100;
disp(strcat(['p-prime max CFD:', num2str(maxpfcfd)]));
disp(strcat(['%-diff from CFD: ', num2str(pf_metric)]));
pause;

% Compute max/min pressure fluctuations from BEM approaches
minpfbem = min(min(pf_bem_history)); minpfcfd = min(min(pf_cfd));

f_cfd_comp = figure; f_cfd_comp.Position = [639 -815 722 475];
max_pf = max([maxpfbem, maxpfcfd]); min_pf = min([minpfbem, minpfcfd]);

subplot(121);
scatter(col_pts(:,1) ./ La, col_pts(:,2) ./ La, sz, pf_bem_history(:,141), 'filled'); grid on;
...
v_fname = strcat(['./video_results/3d/c6/m', m_choice, '_', amp_choice, '_3d_comparison.mp4']);
...
for l = 1:length(t_phys)
  ...
  caxis([minpfbem, maxpfbem]); pbaspect([1 2 1]);
  ax = gca; ax.FontSize = afs;
  xlabel('$x / a$', 'interpreter', 'latex', 'fontsize', fs);
  ylabel('$y / a$', 'interpreter', 'latex', 'fontsize', fs);
  title('BEM', 'interpreter', 'latex', 'fontsize', fs);
  ...
  colorbar; colormap('jet'); caxis([minpfcfd, maxpfcfd]); pbaspect([1 2 1]);
  ax = gca; ax.FontSize = afs;
  xlabel('$x / a$', 'interpreter', 'latex', 'fontsize', fs);
  ylabel('$y / a$', 'interpreter', 'latex', 'fontsize', fs);
  title('CFD', 'interpreter', 'latex', 'fontsize', fs);
