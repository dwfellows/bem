...
f_det = figure; Np = max(size(detA_hist));
for j = 1:2
...
ylabel('Im( $\omega$ )', 'interpreter', 'latex', 'fontsize', fs);
title(strcat(['Solution to Eigenfrequencies, $M = ', num2str(M_list(1)), '$ to $M = ', num2str(M_list(end)), '$, $N_e = ', num2str(N), '$, $N_m = ', num2str(num_modes), '$']), 'interpreter', 'latex', 'fontsize', fs);
