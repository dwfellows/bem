...
% Import structural quantities
elem_node = importdata('~/research/dmdc_panel/stability_comparisons/panel_stability_data/applications/panel_flutter_blaine/blaine_plate/elements.txt'); elem_node = elem_node.data;
node_elem = dlmread('~/research/dmdc_panel/stability_comparisons/panel_stability_data/applications/panel_flutter_blaine/blaine_plate/node_elem.txt');
nodes = dlmread('~/research/dmdc_panel/stability_comparisons/panel_stability_data/applications/panel_flutter_blaine/blaine_plate/nodes.txt');
structural_node_locs = nodes(:,2:4);
surface_node_data = dlmread('~/research/dmdc_panel/stability_comparisons/panel_stability_data/applications/panel_flutter_blaine/blaine_plate/face_nodes.txt');
struct_faces = dlmread('~/research/dmdc_panel/stability_comparisons/panel_stability_data/applications/panel_flutter_blaine/blaine_plate/surf_faces_trim.txt');
mode1_zdir_deform = dlmread('~/research/dmdc_panel/stability_comparisons/panel_stability_data/applications/panel_flutter_blaine/blaine_plate/mode1_zdir_deform.txt');
def_func = mode1_zdir_deform(:,5);
def_func = def_func ./ max(def_func);
...
disp('cleared coordinate computation')
...
if (run_3dstfem == 1)
  ...
  subplot(223);
  scatter(col_pts(:,1) ./ La, col_pts(:,2) ./ La, sz, real(pbar_st) ./ flow.pref, 'filled');
  ax = gca; ax.FontSize = afs;
  colorbar; colormap('jet'); pbaspect([1 2 1]);
  xlabel('$x / a$', 'interpreter', 'latex', 'fontsize', fs);
  ylabel('$y / a$', 'interpreter', 'latex', 'fontsize', fs);
  ...
  subplot(224);
  scatter(col_pts(:,1) ./ La, col_pts(:,2) ./ La, sz, imag(pbar_st) ./ flow.pref, 'filled');
  ax = gca; ax.FontSize = afs;
  colorbar; colormap('jet'); pbaspect([1 2 1]);
  xlabel('$x / a$', 'interpreter', 'latex', 'fontsize', fs);
  ylabel('$y / a$', 'interpreter', 'latex', 'fontsize', fs);
...
if (run_3dstanal == 1)
  ...
  subplot(223);
  scatter(col_pts(:,1) ./ La, col_pts(:,2) ./ La, sz, real(pbar_st_anal) ./ flow.pref, 'filled');
  ax = gca; ax.FontSize = afs;
  colorbar; colormap('jet'); pbaspect([1 2 1]);
  xlabel('$x / a$', 'interpreter', 'latex', 'fontsize', fs);
  ylabel('$y / a$', 'interpreter', 'latex', 'fontsize', fs);
  ...
  subplot(224);
  scatter(col_pts(:,1) ./ La, col_pts(:,2) ./ La, sz, imag(pbar_st_anal) ./ flow.pref, 'filled');
  ax = gca; ax.FontSize = afs;
  colorbar; colormap('jet'); pbaspect([1 2 1]);
  xlabel('$x / a$', 'interpreter', 'latex', 'fontsize', fs);
  ylabel('$y / a$', 'interpreter', 'latex', 'fontsize', fs);
...
% Compute LPT mode
p_bar_lpt = (-i*(amp/2)*flow.gamma*flow.M.*dpsi_dx_st_anal + flow.gamma*k_wav*(amp/2).*psi_st_anal);

figure;
subplot(121);
scatter(col_pts(:,1) ./ L_panel, col_pts(:,2) ./ L_panel, sz, real(p_bar_lpt)); grid on;
ax = gca; ax.FontSize = afs; colorbar; colormap('jet'); pbaspect([1 2 1]);
ylabel('$y / a$', 'interpreter', 'latex', 'fontsize', fs);
xlabel('$x / a$', 'interpreter', 'latex', 'fontsize', fs);
title('Re( $\bar{p}^{\prime}_{LPT} / p_{\infty}$ )', 'interpreter', 'latex', 'fontsize', fs);
subplot(122);
scatter(col_pts(:,1) ./ L_panel, col_pts(:,2) ./ L_panel, sz, imag(p_bar_lpt)); grid on;
ax = gca; ax.FontSize = afs; colorbar; colormap('jet'); pbaspect([1 2 1]);
ylabel('$y / a$', 'interpreter', 'latex', 'fontsize', fs);
xlabel('$x / a$', 'interpreter', 'latex', 'fontsize', fs);
title('Im( $\bar{p}^{\prime}_{LPT} / p_{\infty}$ )', 'interpreter', 'latex', 'fontsize', fs);
...
pf_bem_history = (pbar_prime_3d_anal')*exp(i*omega.*t_phys) + conj( (pbar_prime_3d_anal')*exp(i*omega.*t_phys) );
subplot(121);
...
if (run_3dfem == 1)
  ...
  for l = 1:length(t_phys)
    t_loc = t_phys(l);
    p_loc = pbar_prime*exp(i*omega*t_loc); p_loc = p_loc + conj(p_loc);
...
if (run_3danal == 1)
  ...
  for l = 1:length(t_phys)
    ...
    caxis([min_pf_cfd, max_pf_cfd]); pbaspect([1 2 1]);
    ax = gca; ax.FontSize = afs;
    xlabel('$x / a$', 'interpreter', 'latex', 'fontsize', fs);
    ylabel('$y / a$', 'interpreter', 'latex', 'fontsize', fs);
    title('BEM', 'interpreter', 'latex', 'fontsize', fs);
...
if (run_3dstanal == 1)
  ...
  for l = 1:length(t_phys)
    t_loc = t_phys(l);
    p_loc = pbar_st_anal*exp(i*omega*t_loc); p_loc = p_loc + conj(p_loc);
    z_loc = imag( psi.*(amp*exp(i*omega*t_loc)) );
