...
% Use this testing function to replicate the 2D maximum pressure fluctuation data in dissertation
...
m_choice = '15'; amp_choice = 'origamp';   % for c6: 05, 085, 15, 3; amps: twice/orig/half/fifth
M = str2num(strcat([m_choice(1), '.', m_choice(2:end)]));
amp = 0.000722158;
omega = 376.991;

if (strcmp(amp_choice,'fifthamp'))
  amp = amp / 5;
elseif (strcmp(amp_choice, 'halfamp'))
  amp = amp/2;
...
N = 1601; % Number of points on panel where velocity perturbation is computed
...
% DMD may not be computed yet; worth checking for Mach regimes first. 3/8/23
...
disp('Pausing before video comparison. Press anything to begin process.');
...
if (M < 1)
  pressure_data = dlmread(strcat(['./comparison_data/c6_2d/c6_subsonic_data/m', m_choice, '/', amp_choice, '/surface_pressure_history_m', m_choice, '_c6_', amp_choice, '.csv']));
  y_data = dlmread(strcat(['./comparison_data/c6_2d/c6_subsonic_data/m', m_choice, '/', amp_choice, '/surface_yloc_history_m', m_choice, '_c6_', amp_choice, '.csv']));
else
  pressure_data = dlmread(strcat(['./comparison_data/c6_2d/c6_supersonic_data/m', m_choice, '/', amp_choice, '/surface_pressure_history_m', m_choice, '_c6_', amp_choice, '.csv']));
  y_data = dlmread(strcat(['./comparison_data/c6_2d/c6_supersonic_data/m', m_choice, '/', amp_choice, '/surface_yloc_history_m', m_choice, '_c6_', amp_choice, '.csv']));
end
pressure_data = pressure_data'; y_data = y_data';
...
maxpfbem = max(max(pf_bem_history)); maxpfcfd = max(max(pf_cfd));
pf_metric = ((maxpfbem - maxpfcfd)/maxpfcfd)*100; disp(pf_metric);
pause;
