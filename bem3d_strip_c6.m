...
if (run_3dstanal == 1)
  ...
  for j = 1:(Ny-1)
    ...
    strip_psi = psi_in(strip_indices); loc_amp = amp*max(strip_psi); %strip_psi = strip_psi ./ max(strip_psi);
    strip_dpsi_dx = dpsi_dx_in(strip_indices); strip_dpsi_dx = strip_dpsi_dx ./ max(strip_psi); strip_psi = strip_psi ./ max(strip_psi);
    [strip_phi, strip_pbar] = bem2d(col_x, bnd_x, strip_psi, strip_dpsi_dx, loc_amp, omega, flow);
    phi_st(strip_indices) = strip_phi;
    pbar_st(strip_indices) = strip_pbar;
  end

  f_soln_3d_st = figure; f_soln_3d_st.Position = [236 58 1196 889];
  subplot(221);
  ...
  ylabel('$y / a$', 'interpreter', 'latex', 'fontsize', fs);
  title('$Im \big( \bar{p}^{\prime} / p_{\infty} \big)$', 'interpreter', 'latex', 'fontsize', fs);
  sgtitle(strcat(['Reduced Velocity Perturbation Potential and Pressure Fluctuation, 3D Strip Theory, $M = ', num2str(M), '$']), 'interpreter', 'latex', 'fontsize', fs);
...
pf_3dst_bem_history = (pbar_st.')*exp(i*omega*t_phys); pf_3dst_bem_history = (pf_3dst_bem_history + conj(pf_3dst_bem_history)) ./ flow.pref;

max_pf_bem_st = max(max( pf_3dst_bem_history ));
min_pf_bem_st = min(min( pf_3dst_bem_history ));
