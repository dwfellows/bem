...
m_choice = '085'; amp_choice = 'origamp';
...
% Compute velocity perturbation potential solution
Uinf = M*sqrt(flow.gamma * R * flow.Tref);
pts = [-1 / sqrt(3), 1 / sqrt(3)];
phibar_prime = zeros(N,1); pbar_prime = zeros(N,1);
for ind = 1:N
  ...
  for j = 1:N
    ...
    for l = 1:2
      for m = 1:2
        ...
        [G] = greens3d_subsonic(x,y,0, int_point_loc(1), int_point_loc(2), 0, k_wav, M);
        psi_loc = polyval(p, int_point_loc(1)./La) * polyval(p, int_point_loc(2) ./ Lb);
        dpsi_dx_loc = polyval(p, int_point_loc(2)./Lb) * polyval(pd, int_point_loc(1)./La) ./ La;
...
phibar_prime_3d_anal = phibar_prime;

% Compute pressure solution
dx = col_pts(2,1) - col_pts(1,1);
for l = 1:(Nx-1)
  for m = 1:(Ny-1)
    ind = (m-1)*(Nx-1) + l;
    if (l==1)
      pbar_prime(ind) = -flow.rhoref*( Uinf*((phibar_prime(ind+1) - phibar_prime(ind))/dx) + i*omega*phibar_prime(ind) );
    elseif (l == (Nx-1))
      pbar_prime(ind) = -flow.rhoref*( Uinf*((phibar_prime(ind) - phibar_prime(ind-1))/dx) + i*omega*phibar_prime(ind) );
    else
      pbar_prime(ind) = -flow.rhoref*( Uinf*((phibar_prime(ind+1) - phibar_prime(ind-1))/(2*dx)) + i*omega*phibar_prime(ind) );
    end
  end
end
pbar_prime_3d_anal = pbar_prime;

phibar_prime_3d_anal = phibar_prime_3d_anal.';
pbar_prime_3d_anal = pbar_prime_3d_anal.';
